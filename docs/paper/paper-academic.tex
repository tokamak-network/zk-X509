...
to additionally disclose \texttt{countryHash} via selective disclosure
(Section 3.11).
...
\item
  ...
  multi-wallet registration, automatic identity expiry, selective
  attribute disclosure, CA-anonymous verification via Merkle tree, and
  self-service wallet migration.
\item
...
\texttt{countryHash} & \texttt{bytes32} & UTF-8 plaintext (right-padded) or \texttt{0x0} \\
\texttt{orgHash} & \texttt{bytes32} & UTF-8 plaintext (right-padded) or \texttt{0x0} \\
\texttt{orgUnitHash} & \texttt{bytes32} & UTF-8 plaintext (right-padded) or \texttt{0x0} \\
\texttt{commonNameHash} & \texttt{bytes32} & UTF-8 plaintext (right-padded) or \texttt{0x0} \\
\bottomrule
\end{tabular}
\end{table}
...
crlMerkleRoot, and four selective disclosure fields (countryHash,
orgHash, orgUnitHash, commonNameHash --- UTF-8 plaintext in bytes32 when
...
Prior sections describe a binary identity model: the verifier learns
only ``this wallet holds a valid certificate'' without any attributes.
While this suffices for simple Sybil resistance, real-world applications
often require \textbf{granular attribute verification}: ``this user is
from country X'' or ``this user belongs to organization Y'' --- without
revealing other attributes like name or ID number.
...
field from the certificate's subject DN and commits the UTF-8 plaintext
right-padded to 32 bytes as a public value. For unset bits, zero is
committed. Values exceeding 32 bytes are truncated, which is sufficient
for practical fields (country codes are 2 bytes; Korean organization
names are typically under 30 bytes in UTF-8).

This design intentionally reveals the \emph{actual} field value rather
than a hash, because disclosure means the user consents to public
visibility. Service operators can directly read disclosed fields on-chain
(e.g., filtering by country or distinguishing individual from business
certificates via the Organization field).

\textbf{User sovereignty.} The \texttt{disclosure\_mask} is chosen by
the user at proof generation time, not by the verifier. The same
certificate can produce different proofs for different applications: a
DAO voting contract may require only country disclosure, while a
corporate DeFi protocol may additionally require organization disclosure.
The user decides what to reveal on a per-proof basis.
...
\textbf{Business identity use case.} By requiring disclosure of the
Organization (O) field (\texttt{minDisclosureMask} bit 1), a service can
distinguish individual certificates (\texttt{orgHash} = \texttt{0x0},
since individual certificates lack an Organization field) from business
certificates (\texttt{orgHash} $\neq$ \texttt{0x0}). This enables
policies such as ``only business entities may register.''
